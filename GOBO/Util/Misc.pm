...
sub __compare_objects {
    ...
    if ( reftype($o1) eq 'ARRAY' ) {
        return if scalar @$o1 ne scalar @$o2;
        for my $i ( 0 .. $#$o1 ) {
            return
                unless __compare_objects( $o1->[$i], $o2->[$i] );
        }
        return 1;
    }

    # if they both have IDs, do a quick comparison...
    elsif ( blessed($o1) and ref($o1) =~ /^GOBO::/ )
  ...
  #			print STDERR "$a = " . $a->name . "; accessor = " . $a->accessor . "\n";
    ...
    elsif ( ref $o1 eq 'HASH' ) {    #warn "Running hash tests";
        return 0 if scalar keys %$o1 ne scalar keys %$o2;
        return 0
            if join( "\0", sort keys %$o1 ) ne join( "\0", sort keys %$o2 );
        foreach my $key ( keys %$o1 ) {
            return 0 unless __compare_objects( $o1->{$key}, $o2->{$key} );
        }
        return 1;
    }
    ## other tests...?
